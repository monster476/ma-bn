...
def write_row_to_csv(csv_path, row_dict):
    ...
    all_fields = list(dict.fromkeys(existing_fields + list(row_dict.keys())))
    ...
    with open(csv_path, "a", newline="") as f:
        ...
        complete_row = {k: row_dict.get(k, "") for k in all_fields}
...
class CriticBiasCallback(EvalCallback):
    def __init__(
        ...
        self.csv_path = os.path.join(log_path, "eval.csv")
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and (self.n_calls % self.eval_freq == 0 or self.n_calls == 1):
            ...
            row_dict = {"timesteps": self.num_timesteps}
            for k, v in metrics.items():
                row_dict[f"{k}_mean"] = float(jnp.mean(v))
                row_dict[f"{k}_std"] = float(jnp.std(v))
            write_row_to_csv(self.csv_path, row_dict)
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and (self.n_calls % self.eval_freq == 0 or self.n_calls == 1):
            ...
            row_dict = {
                ...
                "mean_ep_length": float(mean_ep_length)
            }
            if len(self._is_success_buffer) > 0:
                row_dict["success_rate"] = float(np.mean(self._is_success_buffer))
            write_row_to_csv(self.csv_path, row_dict)
