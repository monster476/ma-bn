...
class SAC(OffPolicyAlgorithmJax):
    ...
    def __init__(
            ...
            update_mode: str = "TEETE",
            mix_data: bool = False,
            actor_use_bn: bool = True,
    ) -> None:
        super().__init__(
            policy=policy,
            env=env,
        ...
        self.actor_use_bn=actor_use_bn
    ...
    def _setup_model(self) -> None:
        ...
        if not hasattr(self, "policy") or self.policy is None:
            ...
            self.policy = self.policy_class(  # type: ignore[assignment]
                ...
                actor_use_bn=self.actor_use_bn,
                **self.policy_kwargs,
            )
    ...
    @staticmethod
    @partial(jax.jit, static_argnames=["crossq_style", "td3_mode", "use_bnstats_from_live_net", "update_mode"])
    def update_critic(
            crossq_style: bool,
            td3_mode: bool,
        ...
        if update_mode[1] == "T":
            train1 = True
            dist, actor_state_updates = actor_state.apply_fn(
            ...
            train1 = False
            actor_state_updates = None
            dist = actor_state.apply_fn(
                {"params": actor_state.params, "batch_stats": actor_state.batch_stats},
                next_observations, train=train1
            )
        ...
        def mse_loss(params, batch_stats, dropout_key):
            if not crossq_style:
                ...
                assert update_mode[3] == update_mode[4]
                if update_mode[3] == "T":
                    train34 = True
                else:
                    train34 = False
        ...
        return (qf_state, metrics, key, actor_state_updates)

    @staticmethod
    @partial(jax.jit, static_argnames=["q_reduce_fn", "td3_mode", "update_mode", "mix_data"])
    def update_actor(
            actor_state: ActorTrainState,
            qf_state: RLTrainState,
            ent_coef_state: TrainState,
        ...
        def actor_loss(params, batch_stats):
            ...
            if update_mode[2] == "T":
                ...
                qf_pi, _ = qf_state.apply_fn(
                    ...
                    actor_actions,
                    mutable=['batch_stats'],
                    rngs={"dropout": dropout_key}, train=train2
                )
            else:
                train2 = False
    ...
    @partial(jax.jit, static_argnames=["cls", "crossq_style", "td3_mode", "use_bnstats_from_live_net", "gradient_steps",
                                       "q_reduce_fn", "update_mode", "mix_data"])
    def _train(
            cls,
            crossq_style: bool,
            td3_mode: bool,
        ...
        if actor_state_updates is not None and update_mode[0] == "E":
            actor_state = actor_state.replace(batch_stats=actor_state_updates["batch_stats"])
