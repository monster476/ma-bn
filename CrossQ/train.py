...
parser.add_argument("-actor_use_bn",         type=bool, required=False, default=False, help="Set BN mode.")
parser.add_argument("-update_mode",         type=str, required=False, default="TEETE", help="Set BN mode.")
parser.add_argument("-mix_data",         type=bool, required=False, default=False, help="mix data in step III.")
...
parser.add_argument("-bn",                type=float, required=False, default=True,  choices=[0,1], help="Use batch norm layers in the actor and critic networks")
...
if args.algo == 'droq':
    ...
    if args.dropout: 
        dropout_rate = 0.01
    group = f'SAC_{args.env}_bn({args.bn}/{args.bn_momentum}/{args.bn_mode})_ln{(args.ln)}_xq({args.crossq_style}/{args.tau})_utd({args.utd}/{args.policy_delay})_A{args.adam_b1}_Q({net_arch["qf"][0]})_l{args.lr}_actor_use_bn={args.actor_use_bn}'
...
with wandb.init(
    ...
    model = SAC(
        ...
        tensorboard_log=f"logs/{group + 'seed=' + str(seed) + '_' + str(args.update_mode) + '_actor_use_bn=' + str(args.actor_use_bn)}/",
        update_mode=args.update_mode,
        mix_data=args.mix_data,
        actor_use_bn=args.actor_use_bn,
    )

    # Create log dir where evaluation results will be saved
    eval_log_dir = f"./eval_logs/{group + 'seed=' + str(seed) + '_' + str(args.update_mode) + '_actor_use_bn=' + str(args.actor_use_bn)}/eval/"
    qbias_log_dir = f"./eval_logs/{group + 'seed=' + str(seed) + '_' + str(args.update_mode) + '_actor_use_bn=' + str(args.actor_use_bn)}/qbias/"
