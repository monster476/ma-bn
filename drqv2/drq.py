...
class Actor(nn.Module):
    ...
    def __init__(self, obs_shape, action_shape, hidden_dim, feature_dim, hidden_depth,
                 log_std_bounds):
        super().__init__()
...
class DrQAgent(object):
    ...
    def __init__(self, obs_shape, action_shape, action_range, device, critic_cfg, actor_cfg, discount,
                 init_temperature, lr, use_tb, feature_dim, actor_update_frequency,
                 critic_target_tau, critic_target_update_frequency, batch_size):
        ...
        self.actor = Actor(obs_shape, action_shape, actor_cfg.hidden_dim, feature_dim, actor_cfg.hidden_depth, \
                           actor_cfg.log_std_bounds).to(self.device)

        #self.critic = hydra.utils.instantiate(critic_cfg).to(self.device)
        self.critic = Critic(obs_shape, action_shape, critic_cfg.hidden_dim, feature_dim, critic_cfg.hidden_depth).to(self.device)
        # self.critic_target = hydra.utils.instantiate(critic_cfg).to(
        #     self.device)
        self.critic_target = Critic(obs_shape, action_shape, critic_cfg.hidden_dim, feature_dim, critic_cfg.hidden_depth).to(self.device)
