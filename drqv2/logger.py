...
COMMON_EVAL_FORMAT = [('frame', 'F', 'int'), ('step', 'S', 'int'),
                      ('episode', 'E', 'int'), ('episode_length', 'L', 'int'),
                      ('episode_reward', 'R', 'float'),
                      ('est_bias_mean', 'M', 'float'),
                      ('est_bias_var', 'V', 'float'),
                      ('total_time', 'T', 'time')]
...
class MetersGroup(object):
    ...
    def _dump_to_console(self, data, prefix, writer=None):
        ...
        # print(' | '.join(pieces))
        writer.info(' | '.join(pieces))


    def dump(self, step, prefix, writer=None):
        if len(self._meters) == 0:
            return
        data = self._prime_meters()
        data['frame'] = step
        self._dump_to_csv(data)
        self._dump_to_console(data, prefix, writer)
        self._meters.clear()
...
class Logger(object):
    def __init__(self, log_dir, use_tb):
        ...
        if use_tb:
            self.sw = SummaryWriter(str(log_dir / 'tb'))
        else:
            self.sw = None
    ...
    def dump(self, step, ty=None, writer=None):
        if ty is None or ty == 'eval':
            self._eval_mg.dump(step, 'eval', writer)
        if ty is None or ty == 'train':
            self._train_mg.dump(step, 'train', writer)
...
class LogAndDumpCtx:
    ...
    def __call__(self, key, value):
        self.logger.log(f'{self._ty}/{key}', value, self._step)

    def __exit__(self, *args):
        self.logger.dump(self._step, self._ty, self.logger.writer)
